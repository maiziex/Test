...
void read_pileup(string input_filename,  long long start, long long end,int samplenum)
{
    ifstream input;
        ...
        if(content.find("+")!= std::string::npos ||  content.find("-")!= std::string::npos)
        {
            ...
            for(i = 0; i < content.size(); i++)
                ...
                if((content[i] == '.' || content[i] == ',') && content[i+1] != '+' && content[i+1] != '-')
                {
                    ...
                    {
                        //printf("here 1\n");
                        current_indel_key = to_string(locus) + 'A' + to_string(samplenum);
                    ...
                    {
                        //printf("here 2\n");
                        current_indel_key = to_string(locus) + 'T' + to_string(samplenum);
                        auto it = dict_indel.find(current_indel_key);
                        if(it != dict_indel.end())
                            it->second = dict_indel[current_indel_key]+1 ;
                        else
                    ...
                    {
                        //printf("here 3\n");
                        current_indel_key = to_string(locus) + 'C' + to_string(samplenum);
                        auto it = dict_indel.find(current_indel_key);
                        if(it != dict_indel.end())
                            it->second = dict_indel[current_indel_key]+1 ;
                        else
                    ...
                    {
                        //printf("here 4\n");
                        current_indel_key = to_string(locus) + 'G' + to_string(samplenum);
                        auto it = dict_indel.find(current_indel_key);
                        if(it != dict_indel.end())
                            it->second = dict_indel[current_indel_key]+1 ;
                        else
                ...
                else if(content[i] == '+' || content[i] == '-')
                {
                    ...
                    for(j = i + 1; j < content.size(); j++)
                    {
                        ...
                        else
                        {
                            ...
                            current_indel_key = to_string(locus) + current_indel_key + to_string(samplenum);
...
{  
    ...
    {  
      ...
      if (S_ISDIR( filestat.st_mode ))
        continue;
      read_pileup(filepath,start, end,samplenum);
      samplenum++;
    }
    
   
    ofstream myfile("saved_indel_addcriterion.txt");
    if(myfile.is_open())
    {
